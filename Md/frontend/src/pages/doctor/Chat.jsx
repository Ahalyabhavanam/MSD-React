...
import { ArrowLeft, Send, User, Stethoscope, Phone, Video, FileText, Clock, MessageCircle } from 'lucide-react'
...
const DoctorChat = () => {
  ...
  const loadChatHistory = () => {
    if (selectedAppointment) {
      ...
      const sampleMessages = [
        {
          id: 1,
          sender: selectedAppointment.patient?._id,
          ...
          senderName: selectedAppointment.patient?.name
        },
        {
          id: 2,
          sender: user._id,
          text: `Hello ${selectedAppointment.patient?.name}, I'm here to help you with any questions about your appointment. What would you like to know?`,
  ...
  const handleSend = () => {
    ...
    realtimeService.sendChatMessage({
      ...
      recipientId: selectedAppointment.patient._id,
      recipientName: selectedAppointment.patient.name
    })
  ...
  const getAvailableAppointments = () => {
    const filteredAppointments = appointments.filter(apt => apt.status === 'confirmed' || apt.status === 'pending')
    
    // Remove duplicates based on ID and other unique identifiers
    const uniqueAppointments = filteredAppointments.filter((appointment, index, self) => 
      index === self.findIndex(apt => 
        (apt._id && appointment._id && apt._id === appointment._id) ||
        (apt.id && appointment.id && apt.id === appointment.id) ||
        (apt.patient?.name === appointment.patient?.name && 
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Chat with Patients</h1>
              <p className="text-gray-600">Communicate with your patients about their appointments</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="w-1/3 bg-white border-r border-gray-200">
          ...
          <div className="overflow-y-auto h-full">
            {getAvailableAppointments().map((appointment, index) => (
              <button
                key={`appointment-${appointment._id || appointment.id || index}-${appointment.patient?.name || 'unknown'}-${appointment.date}`}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-600 truncate">
                      {appointment.reason || appointment.symptoms || 'No reason specified'}
                    </p>
            ...
            <>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Patient • Appointment: {new Date(selectedAppointment.date).toLocaleDateString()} at {selectedAppointment.time}
                      </p>
                      <p className="text-sm text-gray-500">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                {messages.map((message, index) => (
                  <div key={`message-${message.id || message._id || index}-${message.timestamp || Date.now()}-${message.sender}`} className={`flex ${message.sender === user._id ? 'justify-end' : 'justify-start'}`}>
